...
$(document).ready(function () {
   ...
   // jQuery("#nextButton").click(continuations);
    jQuery("#productsbutton").click(CreateProductList);
    jQuery("#categorybutton").click(CreateCategoryList);
    jQuery("#FeildButton").click(CreateProductField);
    jQuery("#lookupbutton").click(CreateProductlookupField);
    jQuery("#ListIdbutton").click(GetListId);
});
...
function CreateProductList() {
    ...
    call.done(function (data, textStatus, jqXHR) {
        var message = jQuery("#message");
        message.text("products list created");
    });
    call.fail(function (jqXHR, textStatus, errorThrown) {
        var response = JSON.parse(jqXHR.responseText);
        var message = response ? response.error.message.value : textStatus;
...
function CreateCategoryList() {
    ...
    call.done(function (data, textStatus, jqXHR) {
        var message = jQuery("#message");
        message.text("Category list created");
    });
    call.fail(function (jqXHR, textStatus, errorThrown) {
        var response = JSON.parse(jqXHR.responseText);
        var message = response ? response.error.message.value : textStatus;
...
function CreateProductField() {
    var call = jQuery.ajax({
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/Web/Lists/getbytitle('CategoryList')/fields",
        type: "POST",
        data: JSON.stringify({"__metadata": { type: "SP.Field" },
        FieldTypeKind: 3,         
        Title:"CategoryName",                       
            }),
...
function CreateProductlookupField(items) {
    var call = jQuery.ajax({
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/Web/Lists/getbytitle('ProductList')/fields",
        type: "POST",
        data: JSON.stringify({
    ...
    call.done(function (data, textstatus, jqXHR) {
        var message = jQuery("#message");
        message.text("Lookup field is added");
    });
    call.fail(function (jqXHR, textStatus, errorThrown) {
        var response = JSON.parse(jqXHR.responseText);
        var message = response ? response.error.message.value : textStatus;
...
function GetListId() {
    var call = jQuery.ajax({
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/Web/Lists/getByTitle('ProductList')/?$select=Id",
        type: "POST",
        datatype: "json",
        headers: {
            Accept: "application/json;odata=verbose",
    ...
    call.done(function (data) {
        var items = data.d.Id;
        var message = jQuery("#message");
        message.text("id retreived ");
        CreateProductlookupField(items);
